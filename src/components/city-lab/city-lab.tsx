...
export function CityLab() {
  ...
  return <div className={styles.lab}>
    ...
    <header className={`${styles.header} ${styles.noPrint}`}><Link className={styles.brand} href="/"><span><Building2 size={21} aria-hidden="true" /></span><div><strong>QALA LAB</strong><small>Аким на 5 часов</small></div></Link><nav aria-label="Основная навигация"><button className={!active ? styles.navActive : undefined} onClick={() => navigate(null)}>Кейсы</button><Link href="/sandbox">Песочница</Link><Link href="/challenge">События <ArrowRight size={14} aria-hidden="true" /></Link></nav><span className={styles.headerLabel}><span /> Лаборатория городских решений</span></header>
    <main id="lab-main">
    ...
    </main><footer className={styles.footer}><span>QALA LAB / Аким на 5 часов</span><span>Хорошие решения начинаются с вопросов.</span></footer>
  </div>;
}
