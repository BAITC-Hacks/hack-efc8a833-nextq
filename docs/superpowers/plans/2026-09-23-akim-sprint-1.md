...
- [x] Описать Zod-схему входа с фиксированным enum направлений и строковыми ID.
- [x] Отклонять неизвестные версии набора/правил, неполные/дублированные решения, неизвестные ID, неизвестный район и мероприятие, чьё направление не совпадает с направлением решения.
- [x] Вычислять стоимость только на сервере по `initiativeId`; не принимать цену, эффекты или score от клиента.
- [x] Реализовать `POST`-обработчик, который разбирает JSON, вызывает `runScenario` и сериализует расчёт.
- [x] Добавить явную обработку неверного JSON, доменной ошибки и неожиданного исключения.
- [x] Проверить `pnpm build`; вручную отправить корректный сценарий и варианты с неполным выбором, неизвестным ID и расходом выше лимита.
- [x] Зафиксировать задачу одним коммитом `feat: add validated scenario endpoint`.
...
- [x] Определить JSON-схему Zod для объяснения: непустая строка `summary`, массивы строк `strengths`, `risks`, `tradeoffs` (пустые массивы допустимы).
- [x] Подключить официальный NVIDIA NIM endpoint `https://integrate.api.nvidia.com/v1/chat/completions`; использовать серверный `NVIDIA_MODEL` со значением по умолчанию `nvidia/nemotron-3-nano-30b-a3b`.
- [x] Отправлять только результат с названиями районов/мероприятий, raw effects, фактическими baseline/final/realizedDelta, расходом и AQoL; требовать объяснение по переданным фактам и запрещать модели пересчитывать AQoL.
- [x] Установить конечный таймаут через `AbortSignal.timeout`; проверить структуру ответа и JSON схемой до возврата.
- [x] Обработать отсутствующий ключ, HTTP-ошибку, таймаут и некорректный JSON как `unavailable` без имитации AI-текста; валидный `summary` с пустыми массивами не считать ошибкой.
- [x] Обновить endpoint: возвращать численный результат даже при ошибке AI и передавать отдельный статус/сообщение для AI.
- [x] Проверить `pnpm build`; без ключа проверить API, что итоговый Score возвращается вместе со статусом `unavailable`.
- [x] Зафиксировать задачу одним коммитом `feat: explain scenarios with NVIDIA NIM`.
...
- [x] Реализовать состояние пяти пустых решений; бесплатное мероприятие доступно как явный выбор в каждом направлении.
- [x] Сделать выбор района и мероприятия с отображением цены, эффекта, побочного эффекта и обновляемого остатка.
- [x] Визуально различать пять этапов, выбранные/невыбранные решения и ошибку бюджета; все действия доступны клавиатурой.
- [x] Отправлять только ID решений в `POST /api/scenario` и блокировать повторное нажатие на время расчёта.
- [x] Показать итог AQoL, дельту к исходному состоянию, бюджет, районные значения, сильные стороны, риски и компромиссы.
- [x] Если AI недоступен, сохранить и показать численный результат с явной меткой отсутствующего AI-объяснения.
- [x] Обновить README: архитектурные слои, версии синтетических данных, формулу AQoL, игровые ограничения, требования к Node.js, запуск, `NVIDIA_API_KEY` и `NVIDIA_MODEL`, демонстрационный сценарий.
- [x] Запустить `pnpm test`, `pnpm lint`, `pnpm build`; production HTTP smoke: главная страница возвращает `200`, API без ключа возвращает численный результат и `narration.status=unavailable`.
- [ ] Пройти интерактивный сценарий в браузере: выбрать пять мероприятий, пересчитать AQoL, изменить решение и проверить блокировку бюджета.
- [x] Зафиксировать задачу одним коммитом `feat: complete five-decision simulation flow`.
